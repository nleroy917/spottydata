...
// import layouts
import { ChartCard, Error, Header, SEO } from '../components/layout'

// import control structures
import { FeatureSelector, PlaylistSelector } from '../components/control'
...
export default function Analysis() {
  ...
  useEffect(() => {
    if (authData !== undefined) {
      ...
      axios
        .get(`${process.env.NEXT_PUBLIC_FUNCTIONS_BASE}/analyze_profile`, {
          headers: hdrs,
        })
        .then((res) => {
          ...
          <div className="w-11/12 md:max-w-screen-xl">
            ...
            <div className="w-full my-2">
              <p className="text-4xl font-extrabold md:text-7xl">
                A look at{' '}
                <span className="text-green-500">{profile.display_name}'s</span>{' '}
                profile.
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col flex-wrap lg:flex-row lg:justify-between">
              <div className="w-full lg:w-1/3 lg:mr-2">
                <ChartCard
                  ...
                  tooltip={toolTips.artistNetwork}
                >
                  <ArtistNetwork
              ...
              <div className="w-full lg:w-5/12 lg:ml-2 lg:flex-1">
                <ChartCard
                  title="Track history"
                  tooltip={toolTips.songCalendar}
                >
                  <SongCalendar data={analysis.calendar_coordinates} />
                </ChartCard>
              </div>
            </div>
            <div className="flex flex-col flex-wrap lg:flex-row lg:justify-between">
              ...
              <div className="w-full my-2 lg:w-5/12 lg:ml-2">
                <ChartCard title="Top Genres" tooltip={toolTips.genreChart}>
                  <GenrePieChart data={analysis.top_genres} />
                </ChartCard>
              </div>
            </div>
              ...
              <div className="w-full my-2">
                ...
                >
                  <div className="flex flex-col items-center justify-start mb-4 md:flex-row">
                    ...
                    />
                    <PlaylistSelector
                      defaultSelection={Object.keys(analysis.feature_data)
                        .slice(0, 2)
                        .map((p) => {
                          return {
                            label: p,
